...
function VideoPlayer() {
  ...
  // Автоплей без звука — работает на мобиле
  useEffect(() => {
  ...
  const toggleMute = () => {
    ...
    setMuted(newMuted)
    if (!newMuted) setVolume(videoRef.current.volume || 1)
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Звук — кнопка mute/unmute */}
          <button onClick={toggleMute} className="flex items-center gap-1.5 text-white hover:text-cyan-400 transition flex-shrink-0">
            ...
            <span className="text-xs">{muted ? 'Включить звук' : 'Выкл. звук'}</span>
          </button>
        </div>
      </div>
    </div>
